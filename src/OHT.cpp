...
void CuckooTable::insert_internal(Branch* item, Client* client) {
    // 检查是否已存在（避免重复插入指针）
    if (find(item->id, client) != nullptr) {
        return; 
    }
    ...
    for (int i = 0; i < MAX_KICKS; ++i) {
        ...
        if (!table[p1].occupied) {
            ...
            current_count++;
            // if(item->id == -201) {
            //     std::cout<<"In access level"<< HOTREE_level_ <<" p1: "<<p1 << " seed: "<<client->vec_seed1_[HOTREE_level_]<<" table size"<< table.size()<<std::endl;
            // }
            return;
        }
        ...
        if (!table[p2].occupied) {
            ...
            current_count++;
            // if(item->id == -201) {
            //     std::cout<<"In access level"<< HOTREE_level_ <<" p2: "<<p2 << " seed: "<<client->vec_seed1_[HOTREE_level_]<<" table size"<< table.size()<<std::endl;
            // }
            return;
        }
